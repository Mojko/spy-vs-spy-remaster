...
onready var disconnected = get_node("disconnected");
...
func _ready():
	...
	get_tree().connect("server_disconnected", self, "_on_disconnect_from_server");
...
func _on_disconnect_from_server():
	...
	NetworkGlobals.client_disconnected = true;
	disconnected.show();
